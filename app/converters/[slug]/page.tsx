...
export async function generateStaticParams() {
  return TOOLS.filter((t) => t.category === "converters").map((t) => ({ slug: t.slug }));
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = `${tool.name} - Free Online Converter Tool | Toolqivo`;
  const url = `${BASE}${tool.route}`;
  return {
    title,
    description: tool.description,
    keywords: [tool.name, ...tool.keywords, "free online converter", "toolqivo"],
