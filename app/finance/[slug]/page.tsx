...
export async function generateStaticParams() {
  return TOOLS.filter((t) => t.category === "finance").map((t) => ({ slug: t.slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const tool = TOOLS.find((t) => t.slug === params.slug && t.category === "finance");
  if (!tool) return { title: "Tool Not Found | Toolqivo" };
  const title = `${tool.name} - Free Online Finance Tool | Toolqivo`;
  const url = `${BASE}${tool.route}`;
  return {
    title,
    description: tool.description,
    keywords: [tool.name, ...tool.keywords, "free finance calculator", "toolqivo"],
